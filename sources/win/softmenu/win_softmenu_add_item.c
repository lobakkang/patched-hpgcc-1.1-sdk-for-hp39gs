...
int win_softmenu_add_item(
    win_widget_t *widget, char *label,
    int event_num, void *event_data)
{
    ...
    /*
     * Search for the next available menu entry.
     */
    while ((entry < WIN_SOFTMENU_MAX_ITEMS)
        && (menu->select & (1 << entry)))
    {
        entry++;
    }

    if (entry == WIN_SOFTMENU_MAX_ITEMS)
    {
        /*
         * Oops, no menu entry found.
         */
        return -1;
    }
