...
// $Header: /cvsroot/hpgcc/sources/hplib/stdio/vsprintf.c,v 1.5 2005/02/22 23:50:18 iblank Exp $
...
/*
	TODO: 
		Approximate to C lib standard formats
		=====================================
		
		* Correct prefix interpretation u,l,ll
		* More formats (?)
		* Remove non standard formats b,U,L (superset (?))

*/

int
vsprintf(char *out, const char *fmt, va_list argp)
{
const char *p;
...
int lead_zero, pad, minus, entry, fpdigits, fpscale;
...
for(p = fmt; *p != '\0'; p++) {
	...
	lead_zero = pad = minus = fpdigits = fpscale = 0;
	entry = 1;
	...
	if (*p == 'f' || *p == 'g') {
		...
		int old_fpdigits = get_decimal_digits();
		
		if (fpscale)
			set_decimal_digits(fpdigits);
		s = dtoa(dround(va_arg(argp,double),fpscale ? fpdigits : old_fpdigits), fmtbuf,*p == 'g');
		if (fpscale)
			set_decimal_digits(old_fpdigits);
		goto numeric_puts;
	}
